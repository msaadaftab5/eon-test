...
file_logger = LoggerGenerator.LoggerGenerator().get_logger("file", "s3_writer")
console_logger = LoggerGenerator.LoggerGenerator().get_logger("console", "s3_writer")
def upload_file(file_path, bucket_name, key):
    s3_client = boto3_manager.get_s3_client()
    file_logger.log_debug(f"Upload file from {file_path} to s3://{bucket_name}/{key}")
    s3_client.upload_file(file_path, bucket_name, key)

def write_pandas_dataframe_as_parquet(df, path):
    console_logger.log_debug(f"Writing Data to {path}")
